...
PR #33 `M6.6: app media gesture session TDD` is **implemented / regression-integrated / latest direction repair automated-green before final docs sync / physical retest pending / draft / not merged / not released**.
...
- explicit click or physical DOWN expands; explicit-click ownership lives on the stable outer media-aware root above generic/media and compact/Peek branch replacement;
...
## Latest physical rejection — candidate #1156

Exact candidate `6c2109195042759b951217f489a201a82dd044cd` / CI #1156 / run `31892019346` passed all three canonical automation jobs and was then physically rejected on Mac16,8/macOS 26.6 on 2026-08-15.

The target-Mac video/test had real media playback active and showed the horizontal track gestures reversed relative to the frozen product contract: a physical LEFT/RIGHT gesture selected the opposite track direction. This blocks `NH-MEDIA-GESTURE-003/004`; Peek direction remains unaccepted and requires retest on the repaired candidate. The video does not by itself establish physical haptic feedback, adapter teardown after Quit, permissions or other unobserved gates.

### Direction root cause and focused TDD
...
This documentation sync creates a new source SHA. That exact descendant must independently pass the same three canonical jobs before its source/artifact provenance is frozen for target-Mac direction retesting.
...
Measured budget evidence: app `883039 B`, DMG `555132 B`, executable `580832 B`. Allowance over immutable `v0.1.0`: app `614400 B`, DMG `466944 B`, executable `315392 B`. Only the DMG allowance increased relative to the preceding cumulative envelope, by one 4096-byte review quantum. The one-line direction fix passed this same unchanged size policy in CI #1158.
...
- lifecycle adapter cleanup after real Quit;
...
1. Pass all three canonical CI jobs on this direction-repair documentation-synchronized PR #33 head.
...
4. If direction passes, continue the remaining Hover Peek, exact-edge DOWN, pointer-exit, UP/DOWN settlement, seek, source-icon, lifecycle/process and permission matrix on the same exact candidate.
