...
## Invariant — animation is driven by SwiftUI/Core Animation, not a timer

The bars animate via SwiftUI's `PhaseAnimator`, a declarative, Core-Animation-backed looping mechanism — not `Timer.scheduledTimer`, not `CADisplayLink`, not a `DispatchSourceTimer`, and not app code invoked on a Swift Concurrency timer. `scripts/performance_policy.py`'s runtime audit already forbids exactly those primitives; this view introduces none of them, so no reviewed-exception entry is needed.

`PhaseAnimator` was chosen over the older `.animation(...repeatForever...)` idiom specifically because physical acceptance found the latter could be silently frozen: the horizontal swipe gesture used for next/previous commits its own `withAnimation` transaction, which interrupted the bars' implicit repeating animation and left them stuck until the panel was expanded and collapsed again (which force-recreated the view). `PhaseAnimator` owns its own animation timeline internally and is not vulnerable to an ancestor view's unrelated explicit animation transaction — this is the documented reason Apple introduced it as the modern replacement for perpetual `repeatForever` loops.
