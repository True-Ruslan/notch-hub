...
- `MediaPeekSession` / `ShippingMediaPeekProbe`'s bounded one-shot probing is left untouched; it becomes redundant in the common case now that authoritative data is already live, but remains a correct no-op-safe fallback and is not removed to avoid destabilizing the existing accepted Peek contract.
