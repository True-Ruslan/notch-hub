...
SOURCE_COMMIT="$ACTUAL_SOURCE_COMMIT" bash "$ROOT_DIR/scripts/build-app.sh" release
readonly SHIPPING_APP="$ROOT_DIR/build/NotchHub.app"
readonly SHIPPING_INFO="$SHIPPING_APP/Contents/Info.plist"
readonly SHIPPING_RESOURCES="$SHIPPING_APP/Contents/Resources"
readonly SHIPPING_PROVENANCE="$SHIPPING_RESOURCES/media-transport-provenance.json"

for required in \
  "$SHIPPING_RESOURCES/mediaremote-adapter.pl" \
  "$SHIPPING_RESOURCES/MediaRemoteAdapter.framework" \
  "$SHIPPING_RESOURCES/MediaRemoteAdapter-LICENSE.txt" \
  "$SHIPPING_PROVENANCE"; do
  test -e "$required"
done

if find "$SHIPPING_APP" \( \
  -name 'MediaTransportCandidate' -o \
  -name 'ProductionMediaTransportCandidate.app' -o \
  -name 'MediaRemoteAdapterTestClient' -o \
  -name 'MediaBridgeProbe' -o \
  -name 'MediaBridgeProbe.app' \
\) -print -quit | grep -q .; then
  echo "Shipping NotchHub.app contains development-only media tooling" >&2
  exit 1
fi

codesign --verify --strict "$SHIPPING_RESOURCES/MediaRemoteAdapter.framework"
test "$(/usr/libexec/PlistBuddy -c 'Print :NHSourceCommit' "$SHIPPING_INFO")" \
  = "$ACTUAL_SOURCE_COMMIT"
test "$(/usr/libexec/PlistBuddy -c 'Print :NHAdapterCommit' "$SHIPPING_INFO")" \
  = "$ADAPTER_COMMIT"
test "$(/usr/libexec/PlistBuddy -c 'Print :NHAdapterPatchSHA256' "$SHIPPING_INFO")" \
  = "$PATCH_SHA256"

python3 - "$SHIPPING_PROVENANCE" "$ACTUAL_SOURCE_COMMIT" "$ADAPTER_COMMIT" "$PATCH_SHA256" <<'PY'
import json
import pathlib
import sys
...
if data != expected:
    raise SystemExit(f"unexpected shipping media provenance: {data!r}")
PY
