...
usage() {
  cat <<'EOF'
Usage:
  scripts/run-shipping-media-target-acceptance.sh --dmg PATH [--output-dir PATH]

Runs the frozen M6.4 shipping-media candidate through target-Mac preflight,
60-second steady resource sampling, 10-minute stability sampling, and normal
application termination/orphan verification.
EOF
}
...
python3 - "$OUTPUT_DIR" "$EXPECTED_SOURCE_COMMIT" <<'PY'
...
reports = {
    ...
    for name in ("preflight", "steady", "stability", "teardown")
}
...
summary = {
    ...
    "stabilitySampleCount": reports["stability"]["sampleCount"],
    "parentExited": True,
    "adapterExited": True,
